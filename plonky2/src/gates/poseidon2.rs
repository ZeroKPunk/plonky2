...
use crate::hash::poseidon2::{Poseidon2, WIDTH, ROUND_F_BEGIN, ROUND_F_END, ROUND_P};
...
use crate::util::serialization::{IoResult, Write, Read};
...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for Poseidon2Gate<F, D> {
    ...
    fn serialize(
        &self, 
        _dst: &mut Vec<u8>,
        _common_dataa: &CommonCircuitData<F, D>) -> IoResult<()> {
        Ok(())
    }

    fn deserialize(_src: &mut crate::util::serialization::Buffer,_common_dataa: &crate::plonk::circuit_data::CommonCircuitData<F, D>) -> IoResult<Self>
    where
        Self: Sized {
    ...
    for Poseidon2Generator<F, D>
{
    ...
    fn serialize(&self, dst: &mut Vec<u8>, _common_data: &crate::plonk::circuit_data::CommonCircuitData<F, D>) -> IoResult<()> {
        println!("Poseidon2Generator serialize!!!!!!");
        dst.write_usize(self.row)
    }

    fn deserialize(src: &mut crate::util::serialization::Buffer, _common_data: &crate::plonk::circuit_data::CommonCircuitData<F, D>) -> IoResult<Self>
    where
        Self: Sized {
